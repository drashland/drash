...
export function CSRF () {
  ...
  const csrf = <F>function csrf (request: Drash.Http.Request, response?: Drash.Http.Response): void {
    ...
    if (response) {
      ...
      if (!requestToken) {
        throw new Drash.Exceptions.HttpMiddlewareException(403, "No CSRF token was passed in");
      }

      if (requestToken !== primaryTokenString)  {
        throw new Drash.Exceptions.HttpMiddlewareException(400, "The CSRF tokens do match");
      }
    }
  };
