...
page
    ...
    div.row
        div.col
            ...
            p Take the example below. There are server-level middleware that will execute before the request and after the request. <code>AuthMiddleware</code> and <code>CacheMiddleware</code> will execute before the request. <code>SomeOtherMiddleware</code> will execute after the request.
            p
              code-block-slotted(:header="false" language="typescript")
                template(v-slot:code) {{ data.example_code.location_server_level.contents }}
            h3 Resource-Level Middleware
            p Resource-level middleware is middleware executed by resources. That is, if a resource specifies a list of middleware in its <code>middleware</code> property, then that middleware will be executed. The server cannot execute resource-level middleware. You define resource-level middleware when you create your server object.
            ...
            p
              code-block-slotted(:header="false" language="typescript")
                template(v-slot:code) {{ data.example_code.location_resource_level.contents }}
    div.row
        div.col
            hr
            h2#setting-the-execution-location Setting The Execution Location
            p There are two locations where middleware can be executed:
            ul
                li
                    strong Before the request
            ...
            p You can have server-level middleware execute before and after the request by placing your middleware in the <code>before_request</code> array or <code>after_request</code> array. Take the example below.
            p
                code-block-slotted(:header="false" language="typescript")
                    template(v-slot:code) {{ data.example_code.location_server_level.contents }}
            ...
            p You can have resource-level middleware execute before and after the request by placing your middleware in the <code>before_request</code> array or <code>after_request</code> array in your resource class' <code>middleware</code> property. When defining middleware in a resource, just specify the middleware as a string. The syntax is written this way to prevent bloating your resource class file's code. See <a href="/#/tutorials/middleware/adding-resource-level-middleware">Adding Resource-Level Middleware</a> for more information. Take the example below.
            p
                code-block-slotted(:header="false" language="typescript")
                    template(v-slot:code) {{ data.example_code.location_resource_level.contents }}
    div.row
        div.col
            hr
            h2#sorting-middleware Sorting Middleware
