...
Resources are constructed by the chain, so __do not store per-request state on `this`__ in a resource or middleware. State kept on the instance is shared across every in-flight request.
