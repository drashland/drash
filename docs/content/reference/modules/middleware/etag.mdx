...
to resources using a [resource group](/reference/standard/http/resource-group).
...
Like the other pre-built middleware, `ETag` is a __factory__. Calling it returns a
...
```ts
import { ETagResponseBuilder } from "@drashland/drash/modules/middleware/e_tag/ETagResponse.js";
```

`ETagResponseBuilder` extends
...
It does not override `build()` &mdash; the inherited one applies.
