...
`HTTPError` needs both halves &mdash; `status_code` for the response status and
`status_code_description` for its `statusText` &mdash; and taking them as one object means they
