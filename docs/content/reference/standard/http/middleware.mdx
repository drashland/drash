...
`Middleware` extends [`Resource`](/reference/core/http/resource). It is a [decorator](https://refactoring.guru/design-patterns/decorator), not a separate concept &mdash; which is why it has the same nine HTTP methods and can stand in for the resource it wraps.
...
Sets what this middleware wraps. Called by [`ResourceGroup`](/reference/standard/http/resource-group) at build time — you do not call it yourself.
...
Where `Middleware` exists to intercept, `ResourceProxy` exists to stand in &mdash; it is the shape [`ResourceGroup`](/reference/standard/http/resource-group) generates when it needs a class that behaves like your resource but is a different class.
...
See the [Middleware tutorial](/docs/middleware) for writing them and [Grouping Resources](/docs/resources/grouping-resources) for attaching them.
