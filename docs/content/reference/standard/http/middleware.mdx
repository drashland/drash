...
`Middleware` extends [`Resource`](/reference/core/http/resource). It is a [decorator](https://refactoring.guru/design-patterns/decorator), not a separate concept — which is why it has the same nine HTTP methods and can stand in for the resource it wraps.
...
Where `Middleware` exists to intercept, `ResourceProxy` exists to stand in — it is the shape [`ResourceGroup`](/reference/standard/http/resource-group) generates when it needs a class that behaves like your resource but is a different class.
