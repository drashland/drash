...
These loggers are deliberately minimal, and Drash does not require you to use them. For production logging, the [Error Handling](/docs/error-handling) page shows the shape to aim for — a real logging library in your `.catch()` block, with the message and the error object going to different levels.
