...
`Middleware` __extends `Resource`__. It is a [decorator](https://refactoring.guru/design-patterns/decorator), not a separate concept — which is why it has the same HTTP method surface and why it can stand in for the resource it wraps.
