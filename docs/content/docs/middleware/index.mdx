...
`Middleware` __extends `Resource`__. It is a [decorator](https://refactoring.guru/design-patterns/decorator), not a separate concept &mdash; which is why it has the same HTTP method surface and why it can stand in for the resource it wraps.
...
`.middleware(A, B, C)` nests outermost-first &mdash; each middleware wraps the next, and the
...
Each pre-built middleware module is an instantiated class you pass to `.middleware()`. For example:
...
import { ETag } from "jsr:@drashland/drash/modules/middleware/ETag";
