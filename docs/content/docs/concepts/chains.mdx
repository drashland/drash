...
When you build HTTP applications with Drash, you start by creating the HTTP module's [`Chain`](/reference/modules/http-native) — the only chain that processes HTTP requests. From there, you give it [resources](/docs/concepts/resources) — the classes you use to process requests and respond to them.
...
The HTTP module's `Chain` processes the data you give it. **You are in charge of defining what goes into the chain and what comes out.** Handlers pass along the input you gave them, not a fixed request type. That is why Node can hand the chain a plain context object while Deno hands it a Web `Request`, and both work.
