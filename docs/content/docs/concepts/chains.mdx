...
When you build HTTP applications with Drash you do not assemble a chain yourself — you
use the HTTP module's [`Application`](/docs/concepts/http-application) class, which
...
request type — which is why one chain can take a Web `Request` on one runtime and a plain
...
[the data it requires](/docs/concepts/http-application#handler-requirements).
