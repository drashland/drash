...
When you build HTTP applications with Drash, you start by creating a chain (specifically the [Request Chain](/docs/reference/chain)). From there, you give it [resources](/docs/concepts/resources) — the classes you use to process requests and respond to them.
...
This is deliberate: translating an error into a response is runtime-specific, and Drash does not own your response object. See [HTTPError](/docs/reference/http-error).
...
Retrieve them via [`request.params`](/docs/reference/resource#request-params), and see [Dynamic paths](/docs/reference/resource#dynamic-paths) for the full syntax.
