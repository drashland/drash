...
When you build HTTP applications with Drash, you start by creating the HTTP module's [`Application`](/reference/modules/http-native). Under the hood it assembles the only chain that processes HTTP requests. From there, you give it [resources](/docs/concepts/resources) — the classes you use to process requests and respond to them.
