...
When you build HTTP applications with Drash, you start by creating a chain (specifically the HTTP module's [Chain](/reference/modules/http/chain)). From there, you give it [resources](/docs/concepts/resources) — the classes you use to process requests and respond to them.
...
## The HTTP Module's Chain

The HTTP module's `Chain` assembles the following handlers in this order:
...
Drash's chains process the data you give them. **You are in charge of defining what goes into the chain and what comes out.** Handlers pass along the input you gave them, not a fixed request type. That is why Node can hand the chain a plain context object while Deno hands it a Web `Request`, and both work.
...
This is deliberate: translating an error into a response is runtime-specific, and Drash does not own your response object. See [HTTPError](/reference/core/http-error).
