...
This part of the codebase implements the most functionality for things like processing HTTP requests (e.g., HTTP module's `Chain` class). Modules import other modules, Standard, and Core.
...
The [Concepts > Builders](/docs/concepts/builders) page covers this topic in more detail.
...
The [Concepts > Native vs. Polyfill](/docs/concepts/native-vs-polyfill) page covers this topic in more detail.
...
Read about [chains](/docs/concepts/chains), then build an application with the HTTP module's `Chain` class — the only chain that processes HTTP requests.
