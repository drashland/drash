...
Drash does not send error responses for you. It is not an HTTP server wrapper, so it has no server of its own to respond with. What it does instead is throw a uniform error object &mdash; `HTTPError` &mdash; and reject the promise that `app.handle()` returned. Turning that rejection into a response is your job, and it happens in one place: the `.catch()` block your server handler already has.
...
- __The body is `e.message`, unchecked.__ That message could have come from deep in your own code &mdash; `"Unable to read from users table. User ID 108 does not exist."` &mdash; and sending it hands a client a description of your database.
