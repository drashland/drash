...
Node uses the __polyfill__ entry point, and its `node:http` server does not give you a Web `Request` &mdash; so the resource receives a __context object__ that you build. The chain only requires a readable `url` and `method`; everything else on it is yours.
