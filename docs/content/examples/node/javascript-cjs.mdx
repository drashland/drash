...
Node uses the __polyfill__ entry point, and its `node:http` server does not give you a Web `Request` — so the resource receives a __context object__ that you build. The app's request chain only requires a readable `url` and `method`; everything else on it is yours.
