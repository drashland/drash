...
Because the resource writes to `context.response` directly, `GET()` returns nothing. That is fine here — the response is already committed to the socket by the time the chain resolves. It stops being fine once you wrap the resource in [middleware](/docs/reference/middleware), which needs a return value to pass along.
