...
__The resource's `paths` must match the function's route.__ A function at `api/index.ts`
is served at `/api`, so a resource with `paths = ["/"]` will never match and every request
answers `404`. This is the easiest thing to get wrong here &mdash; it does not come up in the
...
version, so this uses the __polyfill__ entry point.
...
target the Edge runtime, it provides a global `URLPattern`, so the __native__ entry point
applies there instead &mdash; see [About > Concepts > Native vs. Polyfill](/about/concepts/native-vs-polyfill).
