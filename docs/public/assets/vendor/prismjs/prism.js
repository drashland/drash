/* PrismJS 1.15.0
https://prismjs.com/download.html#themes=prism-tomorrow&languages=markup+css+clike+javascript+abap+actionscript+ada+apacheconf+apl+applescript+c+arff+asciidoc+asm6502+csharp+autohotkey+autoit+bash+basic+batch+bison+brainfuck+bro+cpp+aspnet+arduino+cil+coffeescript+clojure+ruby+csp+css-extras+d+dart+diff+django+docker+eiffel+elixir+elm+markup--hdl+nasm+nginx+nim+nix+nsis+objectivec+ocaml+opencl+oz+parigp+parser+pascal+perl+php+php--basic+wasm+wiki+xeora+xojo+xquery+tap */
var _self =
...
!(function(a) {
  ...
  (a.languages.asciidoc = {
    ...
    (a.languages.adoc = a.languages.asciidoc);
})(Prism);
...
(Prism.languages.markdown = Prism.languages.extend("markup", {})),
  ...
  (Prism.languages.markdown.bold.inside.strike =
    Prism.languages.markdown.strike),
  (Prism.languages.markdown.italic.inside.bold = Prism.languages.markdown.bold),
  (Prism.languages.markdown.italic.inside.strike =
    Prism.languages.markdown.strike),
  (Prism.languages.markdown.strike.inside.bold = Prism.languages.markdown.bold),
  (Prism.languages.markdown.strike.inside.italic =
    Prism.languages.markdown.italic),
...
!(function(e) {
  ...
  var t = "[-+*/_~!@$%^=<>{}\\w]+",
    ...
    d = "\\S+(?:\\s+\\S+)*",
    u = {
      pattern: RegExp(i + "[\\s\\S]*" + s),
      lookbehind: !0,
      inside: {
        "rest-vars": { pattern: RegExp("&(?:rest|body)\\s+" + d), inside: p },
        "other-marker-vars": {
  ...
  (e.languages.soy = {
    ...
    e.hooks.add("before-tokenize", function(a) {
      var t = /{{.+?}}|{.+?}|\s\/\/.*|\/\*[\s\S]*?\*\//g,
        ...
        l = "{/literal}",
        r = !1;
      e.languages["markup-templating"].buildPlaceholders(a, "soy", t, function(
        e
      ) {
