...
Deno.test("Native - Using Request/Response", async (t) => {
  await t.step("Accounts /accounts paths = /accounts", async (t) => {
    ...
    await t.step(`GET waits with header x-wait-1 (method: Promise.all())`, async () => {
      ...
      .then(([ res1, res2 ]) => {
        ...
        asserts.assertEquals(res2, "Hello from Accounts.GET(). Didn't wait!");
      });
    });

    await t.step(`GET does not wait for header x-wait-1 request (method: Promise.any())`, async () => {
      return new Promise<void>((resolve) => {
        ...
        .then((req2Response) => {
          asserts.assertEquals(req2Response, "Hello from Accounts.GET(). Didn't wait!");
        });
      });
    });
