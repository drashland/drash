...
class ResourceNoCookie extends Drash.Resource {
  ...
  public GET(_request: Drash.Request) {
    return new Response(csrfWithoutCookie.token, {
  ...
  public POST(_request: Drash.Request) {
    ...
    return new Response("Success; " + csrfWithoutCookie.token);
  }
}
...
class ResourceWithCookie extends Drash.Resource {
  ...
  public GET(_request: Drash.Request) {
    // Give token to the 'view'
    return new Response(csrfWithCookie.token, {
      headers: {
        "set-cookie": `X-CSRF-TOKEN=${csrfWithCookie.token}`,
      },
    });
  }
  ...
  public POST(_request: Drash.Request) {
    // request should have token
    return new Response("Success; " + csrfWithCookie.token);
  }
}
