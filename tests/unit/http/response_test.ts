...
Rhum.testPlan("http/response_test.ts", () => {
  Rhum.testSuite("constructor()", () => {
    Rhum.testCase("Uses the passed in default response output for headers", () => {
    ...
    Rhum.testCase("Uses Accept header for setting the ContentType header when no config", () => {
      const request = members.mockRequest("/", "get")
      const response = new Drash.Http.Response(request)
      Rhum.asserts.assertEquals(response.headers.get("Content-Type"), "application/json")
    })
