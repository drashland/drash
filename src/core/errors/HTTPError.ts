...
import { HTTPStatusCode as HTTPStatusCodeEnum } from "../Enums.ts";
...
export class HTTPError extends Error {
   ...
   *
   * @param proposedStatusCode A valid HTTP status code.
   * @param message (optional) The error message.
   */
  constructor(proposedStatusCode: HTTPStatusCode, message?: string) {
    super(message);

    const registry = StatusCodeRegistry.get(proposedStatusCode) ||
      StatusCodeRegistry.get(HTTPStatusCodeEnum.InternalServerError);
