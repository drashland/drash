...
export class ErrorHandler implements Interfaces.ErrorHandler {
  ...
  public handle(
    ...
    const ret = new HTTPError(HTTPStatusCode.InternalServerError);
    ...
    if (
      ...
      StatusCodeRegistry.has(errorWithCode.code as HTTPStatusCode)
    ) {
      ret.code = errorWithCode.code as HTTPStatusCode;
    }
    ...
    const res = new Response(ret.stack ?? ret.message, {
      status: ret.code,
      statusText: StatusCodeRegistry.get(ret.code)?.description,
    });
