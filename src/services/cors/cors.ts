import { IService, Service, IContext } from "../../../mod.ts";
...
export class CorsService extends Service implements IService {
    ...
    runAfterResource(context: IContext) {
      ...
      if (!this.#config.origin) {
        ...
        // reflect origin
        this.#config.origin = isOriginAllowed(requestOrigin, this.#config.origin) ? requestOrigin : false;
      }
      ...
      if (
        ...
        if (this.#config.exposeHeaders && typeof this.#config.exposeHeaders === "string") {
          context.response.headers.set("Access-Control-Expose-Headers", this.#config.exposeHeaders);
        }
        ...
        if (this.#config.maxAge) {
          context.response.headers.set("Access-Control-Max-Age", this.#config.maxAge.toString());
        }
  
        if (this.#config.allowMethods && typeof this.#config.allowMethods === "string") {
        ...
        if (!this.#config.allowHeaders) {
          this.#config.allowHeaders = context.request.headers.get("Access-Control-Request-Headers") ||
            undefined;
        }
  
        if (this.#config.allowHeaders) {
          context.response.headers.set("Access-Control-Allow-Headers", this.#config.allowHeaders);
        }
