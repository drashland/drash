...
import { HTTPOptions, HTTPSOptions, STATUS_TEXT, Status, serve, serveTLS } from "../../deps.ts";
...
export class Server {
    ...
    public async handleHttpRequest(request: any): Promise<any> {
        ...
        try {
            this.logger.info(`Request received: ${request.method.toUpperCase()} ${request.url}`);
            ...
            this.logDebug("Using `" + resource.constructor.name + "` resource class to handle the request.");
     ...
     *     See `Drash.Http.Response.send()`.
     */
    public handleHttpRequestError(request: any, error: any, resource: Drash.Http.Resource | any = {}, response: Drash.Http.Response | any = {}): any {
        this.logDebug(`Error occurred while handling request: ${request.method} ${request.url}`);
        ...
        this.logDebug(`Sending response. Content-Type: ${response.headers.get("Content-Type")}. Status: ${response.getStatusMessageFull()}.`);
     ...
     * @return void
     */
    protected executeMiddlewareServerLevelBeforeRequest(request: any): void {
        // Execute server-level middleware
        if (this.middleware.before_request != null) {
            this.middleware.before_request.forEach((middlewareClass: any) => {
     ...
     * @return void
     */
    protected executeMiddlewareServerLevelAfterRequest(request: any, resource: Drash.Http.Resource | null, response: Drash.Http.Response | null): void {
        if (this.middleware.after_request != null) {
            this.middleware.after_request.forEach((middlewareClass: any) => {
