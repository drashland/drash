...
import { SearchIndex } from "../../../standard/handlers/RequestHandlers/SearchIndex.ts";
import { StatusCode } from "../../../standard/http/response/StatusCode.ts";

// Imports > Modules
import { ResourceClassesArray } from "../../base/ResourceClassesArray.ts";
...
export class ResourcesIndex extends SearchIndex<Promise<SearchResult | null>> {

  #cached_queries: Record<string, SearchResult | null> = {};

  #database: Database[] = [];

  #logger: Logger = ConsoleLogger.create("Resources", Level.Off);

  constructor(...resources: ResourceClassesArray) {
    super();

    this.buildIndex(resources);
  }

  public handle(request: { url: string }): Promise<SearchResult | null> {
    return Promise
      .resolve()
      .then(() => this.#logger.debug(`Input received`))
      .then(() => this.search(request))
      .then((result) => super.handle({ request, result }));
  }
  ...
  protected search(request: { url: string }): Promise<SearchResult | null> {
    ...
    if (!("url" in request) || typeof request.url !== "string") {
      throw new HTTPError(StatusCode.InternalServerError, "Request URL could not be read");
    }
    ...
    if (this.#cached_queries[fullyQualifiedUrl]) {
      const cachedResult = this.#cached_queries[fullyQualifiedUrl];
    ...
    for (const resourcePathPatterns of this.#database.values()) {
      for (const pattern of resourcePathPatterns.path_patterns) {
        ...
        this.#cached_queries[fullyQualifiedUrl] = {
          path_params: result.pathname?.groups,
          resource,
        };

        return Promise.resolve(this.#cached_queries[fullyQualifiedUrl]);
      }
    }

    this.#cached_queries[fullyQualifiedUrl] = null;
    return Promise.resolve(this.#cached_queries[fullyQualifiedUrl]);
  }
}