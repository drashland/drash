...
members.test("TemplatEngineResource", async () => {
  ...
  members.assert.equals(
    await response.text(),
    `<!DOCTYPE html> <html class=\"h-full w-full\">   <head>     <meta charset=\"utf-8\"/>     <meta name=\"viewport\" content=\"width=device-width, minimum-scale=1.0, user-scalable=no\"/>     <link rel=\"stylesheet\" href=\"https://cdn.jsdelivr.net/npm/tailwindcss/dist/tailwind.min.css\">     <title>Skills</title>   </head>   <body>     <div style=\"max-width: 640px; margin: 50px auto;\">       <h1 class=\"text-5xl\">Steve Rogers</h1>       <h2 class=\"text-4xl\">Skills</h2>       <ul>      <li>Shield Throwing</li>      <li>Bashing</li>      <li>Hammer Holding</li>    </ul>        Footer      </div>   </body> </html> `
  );
});
