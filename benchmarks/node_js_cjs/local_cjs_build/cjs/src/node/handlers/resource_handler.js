...
const resource_handler_js_1 = require(
  "../../core/handlers/abstract/resource_handler.js",
);
 ...
 */
class ResourceHandler extends resource_handler_js_1.AbstractResourceHandler {
  /**
  ...
  getOriginalUrlPatterns() {
    ...
    for (let path of this.original.paths) {
      ...
      if (typeof path != "string") {
        throw new Error(`Path '${path}' needs to be a string.`);
      }
      let paths;
  ...
  getResourcePathsUsingOptionalParams(path) {
    ...
    return {
      ...
      params: (path.match(REGEX_URI_MATCHES) || []).map((element) => {
  ...
  getResourcePathsUsingWildcard(path) {
    return {
      og_path: path,
      regex_path: `^.${
        path.replace(REGEX_URI_MATCHES, REGEX_URI_REPLACEMENT)
      }/?$`,
      params: (path.match(REGEX_URI_MATCHES) || []).map((element) => {
        return element.replace(/:|{|}/g, "");
      }),
